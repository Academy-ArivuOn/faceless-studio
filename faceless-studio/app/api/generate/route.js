...
export async function POST(request) {
  ...
  try {
    ...
    // Merge channel overrides into cleanInput (channel values win)
    const mergedInput = mergeChannelWithInput(channel, cleanInput);

    // Build final DNA block — channel DNA appended to profile DNA
    ...
    const [saveResult, streakResult, usageResult] = await Promise.allSettled([
      saveGeneration({
        userId:     user.id,
        input:      { ...mergedInput, channel_id: channel?.id || null },
        research,
        creator,
        publisher,
