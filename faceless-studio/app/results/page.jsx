  ...
  render() {
    if (this.state.hasError) return (
      <div style={{ padding: '32px 24px', textAlign: 'center', background: '#FEF2F2', borderRadius: 12, border: '1px solid #FCA5A5', margin: '24px 0' }}>
        ...
        <p style={{ fontSize: 12, color: '#6B7280', marginBottom: 16, fontFamily: "'DM Sans',sans-serif" }}>Content was generated. Try refreshing.</p>
        <button onClick={() => this.setState({ hasError: false })}
          style={{ background: '#0A0A0A', color: '#fff', border: 'none', borderRadius: 8, padding: '8px 18px', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: "'DM Sans',sans-serif" }}>
          Try again
        </button>
      </div>
    );
...
function Label({ children, color = '#5C5C5C' }) {
  return (
...
function Card({ children, highlight = false, style = {} }) {
  return (
    <div style={{
      ...
      border: `1px solid ${highlight ? '#D4A847' : '#E8E8E8'}`,
...
function SceneCard({ scene }) {
  ...
  const TYPE_COLORS = {
    HOOK: '#D4A847', INTRO: '#2563EB', MAIN_CONTENT: '#0E7C4A',
    PATTERN_INTERRUPT: '#F59E0B', RETENTION_BRIDGE: '#8B5CF6',
    CTA: '#EC4899', OUTRO: '#8C8C8C',
  };
  const col = TYPE_COLORS[scene.section_type] || '#0E7C4A';

  return (
    <div style={{ border: '1px solid #E8E8E8', borderRadius: 10, overflow: 'hidden', marginBottom: 8, background: '#FFFFFF' }}>
      <div
        style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', cursor: 'pointer', background: open ? '#F8F8F8' : '#FFFFFF' }}
        onClick={() => setOpen(!open)}>
        <div style={{ width: 32, height: 32, borderRadius: 7, background: `${col}15`, border: `1.5px solid ${col}40`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 800, color: col, flexShrink: 0, fontFamily: "'JetBrains Mono',monospace" }}>
          S{scene.scene_number}
        </div>
        <div style={{ flexShrink: 0, minWidth: 100 }}>
          <div style={{ fontSize: 9, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: col, fontFamily: "'JetBrains Mono',monospace" }}>{scene.section_type?.replace(/_/g, ' ')}</div>
      ...
      {open && (
        <div style={{ padding: '0 16px 16px', borderTop: '1px solid #F0F0F0', display: 'flex', flexDirection: 'column', gap: 12, background: '#FAFAFA' }}>
          <div style={{ marginTop: 14 }}>
            ...
            <div style={{ marginTop: 6, background: '#FFFFFF', borderLeft: `3px solid ${col}`, borderRadius: '0 8px 8px 0', padding: '10px 14px', fontSize: 13, color: '#1A1A1A', lineHeight: 1.75, fontFamily: "'DM Sans',sans-serif" }}>
              {scene.voiceover}
            </div>
          </div>
...
function SprintTracker({ plan = [] }) {
  ...
  const TYPE_COLORS = {
    'Main Video': '#D4A847', 'Short': '#2563EB', 'Reel': '#EC4899',
    'Story': '#8B5CF6', 'Community Post': '#0E7C4A', 'Blog': '#F59E0B', 'Podcast': '#0D9488',
  };
        ...
        return (
          <div key={item.day}
            ...
            {item.repurpose_from && item.repurpose_from !== 'Original' && (
              <span style={{ fontSize: 10, color: '#BCBCBC', fontFamily: "'JetBrains Mono',monospace", flexShrink: 0 }}>↩ {item.repurpose_from}</span>
            )}
          </div>
        );
      })}
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .rp-meta-tag { display: inline-flex; align-items: center; gap: 4px; padding: 4px 11px; border-radius: 6px; font-size: 12px; font-weight: 500; background: #F5F5F5; border: 1px solid #E8E8E8; color: #5C5C5C; font-family: 'DM Sans', sans-serif; }
        ...
        .rp-hook { font-size: 14px; color: #5C5C5C; line-height: 1.7; border-left: 3px solid #0A0A0A; padding-left: 16px; font-style: italic; max-width: 680px; margin-bottom: 20px; font-family: 'DM Sans', sans-serif; }
        .rp-stats { display: flex; gap: 20px; flex-wrap: wrap; }
        .rp-stat { display: flex; align-items: center; gap: 6px; font-size: 13px; color: #5C5C5C; font-family: 'DM Sans', sans-serif; }
        ...
        .rp-tab { padding: 13px 16px; font-size: 13px; font-weight: 500; font-family: 'DM Sans', sans-serif; color: #8C8C8C; background: transparent; border: none; border-bottom: 2px solid transparent; cursor: pointer; white-space: nowrap; transition: all 0.18s; margin-bottom: -1px; display: flex; align-items: center; gap: 5px; }
        ...
        .sp-body { background: #FFFFFF; border: 1px solid #E8E8E8; border-radius: 0 0 10px 10px; padding: 20px 20px; margin-bottom: 20px; }
          ...
          <TabErrorBoundary key={activeTab}>
            ...
            {activeTab === 'script' && (
              <div className="rp-pane">
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 28 }}>
                  ...
                  {creator.creator_mode && <Chip label={`🎬 ${creator.creator_mode}`} color="#2563EB" bg="#EFF6FF" border="#BFDBFE" />}
                </div>
                {creator.shorts_script && (
                  <Section title="⚡ Shorts / Reels Version (60s)" copyText={creator.shorts_script}>
            ...
            {activeTab === 'youtube' && (
              <div className="rp-pane">
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 28 }}>
                    {yt.end_screen_recommendation && (
                      <Card>
                        ...
                        <p style={{ fontSize: 12, color: '#5C5C5C', marginTop: 8, marginBottom: 0, lineHeight: 1.6, fontFamily: "'DM Sans',sans-serif" }}>{yt.end_screen_recommendation}</p>
                      </Card>
                    )}
                    {yt.card_recommendation && (
                      <Card>
                        <Label>🃏 Cards</Label>
                        <p style={{ fontSize: 12, color: '#5C5C5C', marginTop: 8, marginBottom: 0, lineHeight: 1.6, fontFamily: "'DM Sans',sans-serif" }}>{yt.card_recommendation}</p>
                      </Card>
                    )}
                  </div>
                )}
                {bl.seo_title && (
                  <>
                    <div style={{ borderTop: '1px solid #E8E8E8', paddingTop: 24, marginBottom: 20 }}>
                      <Label>✍️ Blog / SEO Assets</Label>
                    </div>
                    ...
                    <div className="sp-body">
                      ...
                      {Array.isArray(ig.hashtags) && ig.hashtags.length > 0 && (
                        <Section title={`Hashtags (${ig.hashtags.length})`} copyText={ig.hashtags.join(' ')}>
                          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                            {ig.hashtags.map((h, i) => <Chip key={i} label={h.startsWith('#') ? h : `#${h}`} color="#EC4899" bg="#FDF2F8" border="#FBCFE8" />)}
                          </div>
                        </Section>
                      )}
                    </div>
                ...
                {tt.caption && (
                  <>
                    ...
                    <div className="sp-body">
                      ...
                      {tt.duet_stitch_suggestion && <Section title="Duet / Stitch Hook" copyText={tt.duet_stitch_suggestion}><Prose style={{ fontSize: 13, padding: '10px 14px' }}>{tt.duet_stitch_suggestion}</Prose></Section>}
                      ...
                      {tt.trending_sound_category && <Section title="Sound Category"><Chip label={tt.trending_sound_category} color="#7C3AED" bg="#F5F3FF" border="#DDD6FE" /></Section>}
                    </div>
                  </>
                )}
                {Array.isArray(publisher.cross_platform_repurpose) && publisher.cross_platform_repurpose.length > 0 && (
                  <Section title="♻️ Repurpose Across Platforms">
                    {publisher.cross_platform_repurpose.map((r, i) => (
                      <div key={i} style={{ padding: '12px 14px', background: '#F8F8F8', border: '1px solid #E8E8E8', borderRadius: 8, marginBottom: 8 }}>
                        <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginBottom: 6, flexWrap: 'wrap' }}>
                          <Chip label={r.platform} />
                          {r.format && <Chip label={r.format} color="#7C3AED" bg="#F5F3FF" border="#DDD6FE" />}
                        </div>
                        <p style={{ fontSize: 12, color: '#5C5C5C', margin: 0, lineHeight: 1.6, fontFamily: "'DM Sans',sans-serif" }}>{r.specific_angle}</p>
                      </div>
                    ))}
                  </Section>
                )}
                {ps.primary_post_day && (
                  <Section title="🕐 Posting Schedule">
                    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>
                      ...
                      {ps.follow_up_story_timing && <Chip label={`+📱 ${ps.follow_up_story_timing}`} />}
                    </div>
